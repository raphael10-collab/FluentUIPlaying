...
contextBridge.exposeInMainWorld('openFileEAPI',{
  openFile: () => ipcRenderer.invoke('dialog:openFile')
})

contextBridge.exposeInMainWorld('closeTabEAPI',{
  closeTab: (id) => ipcRenderer.send('close-tab', id)
})

contextBridge.exposeInMainWorld('canGoEAPI',{
  canGo: (id, direction) => ipcRenderer.invoke('can-go', id, direction)
})

contextBridge.exposeInMainWorld('activeTabEAPI',{
  activeTab: (id) => ipcRenderer.send('active-tab', id)
})

contextBridge.exposeInMainWorld('loadUrlEAPI',{
  loadUrl: (id, url) => ipcRenderer.send('load-url', id, normalizeURL(url))
})
